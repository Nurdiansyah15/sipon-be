...
func (d *SchedulerDispatcher) WithOutboxMode(repo outboxRepo.Repository, transactor *database.Transactor) *SchedulerDispatcher {
	d.mode = ModeOutbox
	d.outboxRepo = repo
...
func (d *SchedulerDispatcher) dispatchOutbox(ctx context.Context, j *entity.ScheduledJob, now time.Time) error {
	...
	return d.transactor.WithTx(ctx, func(txCtx context.Context) error {
		entry := outboxEntity.NewOutboxEntry(j.Type, j.Payload, "")
		if err := d.outboxRepo.Save(txCtx, entry); err != nil {
			return err
		}
