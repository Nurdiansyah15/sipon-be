...
func TestNewActivitySchedule(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s, err := entity.NewActivitySchedule(tt.id, tt.periodID, tt.typ, tt.startTime, tt.endTime, nil, nil, 0, 0, 0)
			if tt.wantErr {
				if err == nil {
...
func TestActivityScheduleUpdate(t *testing.T) {
	s, _ := entity.NewActivitySchedule("sch-1", "ap-1", constant.ActivityScheduleTypeWeekly, "19:30:00", "21:00:00", nil, nil, 0, 0, 0)

	if err := s.Update("20:00:00", "21:30:00", nil, nil, nil, nil, nil); err != nil {
	...
	if err := s.Update("21:00:00", "20:30:00", nil, nil, nil, nil, nil); err == nil {
		t.Error("expected error when end before start")
	}

	if err := s.Update("19:00", "21:00:00", nil, nil, nil, nil, nil); err == nil {
