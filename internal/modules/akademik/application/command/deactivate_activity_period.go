...
type DeactivateActivityPeriodUseCase struct {
	activityPeriodRepo apRepo.ActivityPeriodRepository
}

func NewDeactivateActivityPeriodUseCase(activityPeriodRepo apRepo.ActivityPeriodRepository) *DeactivateActivityPeriodUseCase {
	return &DeactivateActivityPeriodUseCase{activityPeriodRepo: activityPeriodRepo}
}

func (uc *DeactivateActivityPeriodUseCase) Execute(ctx context.Context, id string) (*dto.ActivityPeriodResponse, error) {
	ap, err := uc.activityPeriodRepo.FindByID(ctx, id)
	if err != nil {
		return nil, application.WrapRepoErr(err, constant.CodeActivityPeriodNotFound)
	}
	if err := ap.Deactivate(); err != nil {
