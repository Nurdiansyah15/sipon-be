...
func (s *UserSeeder) Run(ctx context.Context, db *sql.DB) error {
	...
	err := db.QueryRowContext(ctx, `SELECT COUNT(*) FROM user_identities WHERE kind = $1 AND value = $2 AND deleted_at IS NULL`,
		string(domain.LoginIdentifierKindUsername), "usergod").Scan(&count)
	if err != nil {
		return err
	}
	if count > 0 {
	...
	`, userID, "usergod", "Developer / Vendor", "usergod@sipon.dev", string(domain.UserStatusActive))
	if err != nil {
		return err
	}
	...
	_, err = tx.ExecContext(ctx, `
		...
		VALUES ($1, $2, $3, $4, $5, NOW(), NOW())
	`, credID, userID, string(domain.CredentialTypeLocal), string(hashedPassword), true)
	if err != nil {
		return err
	}
	...
	`, identEmailID, userID, credID, string(domain.LoginIdentifierKindEmail), "usergod@sipon.dev", string(domain.LoginIdentityStatusVerified), true)
	if err != nil {
		return err
	}
	...
	_, err = tx.ExecContext(ctx, `
		INSERT INTO user_identities (id, user_id, credential_id, kind, value, status, is_primary, verified_at, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW(), NOW())
	`, identUsernameID, userID, credID, string(domain.LoginIdentifierKindUsername), "usergod", string(domain.LoginIdentityStatusVerified), false)
	if err != nil {
		return err
	}

	var roleID string
	err = tx.QueryRowContext(ctx, `SELECT id FROM roles WHERE name = $1`, string(domain.UserGodRoleName)).Scan(&roleID)
	if err != nil {
		return err
	}
	...
	`, userID, roleID, string(domain.ScopeTypeGlobal), "system")
	if err != nil {
		return err
	}
